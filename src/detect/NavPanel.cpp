...
double NavPanelDetector::match(ClassifyEnv &env) {
    if (forceDetect.empty() || forceDetect == "lpline") {
    ...
    for (int l=2; l < detectors.size(); l++) {
        auto* ldet = dynamic_cast<LineDetector*>(detectors[l].get());
        if (forceDetect.empty() || (ldet && ldet->name == forceDetect)) {
            double lineMatch = detectors[l]->match(env);
            if (lineMatch > 0.5)
                return 1;
        }
    }
