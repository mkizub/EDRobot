...
namespace ed
{
   ...
   struct small_map
   {
     ...
     private:
      ...
      static uint32_t hash_key(std::string_view key) noexcept { return glz::sweethash::sweet32(key); }
     ...
     public:
      ...
      small_map(std::initializer_list<value_type> init)
      {
         reserve(init.size());
      ...
      small_map(small_map&& other) noexcept
      ...
      void swap(small_map& other) noexcept
      {
         using std::swap;
      ...
      friend void swap(small_map& lhs, small_map& rhs) noexcept { lhs.swap(rhs); }
      ...
      // Comparison
      bool operator==(const small_map& other) const
      {
      ...
      bool operator!=(const small_map& other) const { return !(*this == other); }
   };
}
